...
The current image is based on the [openjdk:8-jdk-alpine image](https://hub.docker.com/_/openjdk/), and adds [Terracotta 5.8.0 OSS on top of it](http://terracotta.org/downloads/open-source/catalog)

#### How to start your Terracotta Server(s) in Docker containers

##### Quick start : one active node

    docker run --name tc-server -p 9410:9410 -d terracotta/terracotta-server-oss:5.8.0
...
To build this [Dockerfile](https://github.com/Terracotta-OSS/docker/blob/master/5.8.0/server/Dockerfile), clone this [git repository](https://github.com/Terracotta-OSS/docker) and run :
    ...
    $ cd 5.8.0/server
    $ docker build -t terracotta-server-oss:5.8.0 .